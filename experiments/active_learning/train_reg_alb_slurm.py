...
mkdir(f'{args.output_folder}/{name}/')
...
r_hat = 0
r_tilde = np.zeros(M)
r_tilde_lure = np.zeros(M)
alb_unweighted = np.zeros(M)
alb_weighted = np.zeros(M)
...
if args.inference in ['MFVI', 'Dropout', 'SGD']:
    marginal_loglike_estimate, train_mean_predictive_loglike, dev_mean_predictive_loglike, err_train, err_dev, \
        ...
        train_fc_baseline(net, f'{name}/{j}', args.output_folder, batch_size, epochs, trainloader, valloader, cuda=cuda,
    ...
    marginal_loglike_estimate, train_mean_predictive_loglike, dev_mean_predictive_loglike, err_train, err_dev, \
        approx_d_posterior, true_d_posterior, true_likelihood, exact_ELBO, basedir = \
        train_fc_DUN(net, f'{name}/{j}', args.output_folder, batch_size, epochs, trainloader, valloader,
...
for i in range(M):
    ...
    for x, y, idx in valloader_al:
        r_tilde[i] += compute_loss_DUN(net, x, y, testset_al.M)
        r_tilde_lure[i] += compute_loss_DUN(net, x, y, testset_al.M, idx, testset_al)

alb_unweighted = r_hat - r_tilde
alb_weighted = r_hat - r_tilde_lure

# Save run results
results = {
    'alb_unweighted': alb_unweighted[:M].tolist(),
    'alb_weighted': alb_weighted[:M].tolist(),
    'r_hat': r_hat.tolist(),
    'r_tilde': r_tilde[:M,].tolist(),
    'r_tilde_lure': r_tilde_lure[:M,].tolist()
}
