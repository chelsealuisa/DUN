...
parser.add_argument('--iters', type=int, 
                    help='number of iterations for active learning (default: 15)', default=15)
parser.add_argument('--n_acquired', type=int, 
                    help='number of acquired data points in active learning (default: 100)',
                    default=15)
...
# 15/70/15 split for train/pool/test sets
X_train, X_pool, y_train, y_pool = train_test_split(X_train, y_train, test_size=0.85, random_state=42)
X_pool, X_test, y_pool, y_test = train_test_split(X_pool, y_pool, test_size=0.5, random_state=42)

trainset = Datafeed(torch.Tensor(X_train), torch.Tensor(y_train), transform=None)
poolset = Datafeed(torch.Tensor(X_pool), torch.Tensor(y_pool), transform=None)
valset = Datafeed(torch.Tensor(X_test), torch.Tensor(y_test), transform=None)
print(X_train.shape, y_train.shape, X_pool.shape, y_pool.shape, X_test.shape, y_test.shape)
"""
if args.dataset == 'my_1d':
    valset = Datafeed(torch.Tensor(X_test), torch.Tensor(y_test), transform=None)
...
poolloader = torch.utils.data.DataLoader(poolset, batch_size=batch_size, shuffle=True, pin_memory=True,
                                          num_workers=args.num_workers)                
...
while i<=args.iters:
    ...
    # 2. Record performance of current model / training progress plots
