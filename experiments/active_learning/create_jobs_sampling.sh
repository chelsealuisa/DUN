...
script="train_${group}_slurm_sampling.py"
...
do
    # Create a fresh file
    > ${dataset}_sampling_jobs.txt
    for rep in `seq 0 29`
    do
        echo "${script} --output_folder ${output_folders[${group}]} --inference ${inference} --N_layers ${layers[$inference]} --overcount 1 --width 100 --n_epochs ${epochs} --dataset ${dataset} --lr 0.001 --wd 0.0001 --n_queries ${n_queries[$dataset]} --query_size ${query_size[$dataset]} --init_train ${init_train[$dataset]} --query_strategy variance --clip_var --sampling --T 100 --run_id ${rep}" >> ${dataset}_sampling_jobs.txt
    done
done
