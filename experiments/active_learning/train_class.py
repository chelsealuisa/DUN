...
parser.add_argument('--dataset', help='which dataset to train on', default='spirals',
                    choices=['spirals'])
parser.add_argument('--data_size', type=int, help='size of dataset (default: 2000)',
                    default=2000)
parser.add_argument('--n_arms', type=int, help='number of spiral classes (default: 2)',
                    default=2)
...
name = '_'.join([args.inference, args.dataset, str(args.data_size), str(args.n_arms), str(args.N_layers), 
...
for j in range(n_runs):
    ...
    plt.plot(x, 1 - test_err[:,j])
    plt.xlabel('Train set size')
    plt.ylabel('Validation accuracy')
    plt.tight_layout()
    plt.savefig(f'{args.savedir}/{name}/{j}/val_accuracy.pdf', format='pdf', bbox_inches='tight')
    with open(f'{args.savedir}/{name}/{j}/val_accuracy.pickle', 'wb') as output_file:
                pl.dump(fig_handle, output_file)
    plt.close(plt.gcf())
...
plot_al_results(f'{args.savedir}/{name}/rmse_plot', name, means.reshape(-1), stds.reshape(-1), args.n_queries, args.query_size, args.init_train, measure='accuracy')
