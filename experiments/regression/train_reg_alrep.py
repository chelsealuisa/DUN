...
from src.utils import Datafeed, DatafeedIndexed, mkdir
...
parser.add_argument('--dataset', type=str, help='dataset to train on',
                    choices=["flights"]+uci_names+uci_gap_names, default='boston')
...
parser.add_argument('--method', type=str, help='inference method',
                    choices=['DUN_prior', 'DUN_none', 'MFVI', 'SGD', 'Dropout', 'DUN_wd',
                             'DUN_wd-MLP', 'DUN_none-MLP'], default='DUN_wd')
...
def train_loop(net, trainloader, testloader, epochs, save_path):
    ...
    # Posterior plots
    approx_d_posterior = net.prob_model.current_posterior.data.cpu().numpy()
    exact_posterior, _ = net.get_exact_d_posterior(trainloader, train_bn=True,
                                                                                 logposterior=False)
    true_d_posterior = exact_posterior.data.cpu().numpy()

    x = np.array([i for i in range(10 + 1)])
    ...
    plt.xlabel('Layer')
    plt.savefig(f'{save_path}/depth_post_true.png', format='png', bbox_inches='tight')
    plt.close()
    ...
    plt.xlabel('Layer')
    plt.savefig(f'{save_path}/depth_post_approx.png', format='png', bbox_inches='tight')
    plt.close()
...
def main(args):
    ...
    epochs = 1000 #int(config["best_itr"])

    save_path = f"{results_dir}/{dataset}/{split}/{valprop}/{method}/{network}/{width}/{batch_size}/340_{num}"
    mkdir(save_path)

    # create data
    #trainset, testset, N_train, input_dim, output_dim = get_dset_split(dataset, split, data_dir)
    X_train, X_test, _, _, y_train, y_test, y_means, y_stds = \
        load_gap_UCI(base_dir=data_dir, dname=dataset, n_split=int(split), gap=False)

    # replicate AL dataset
    trainset = DatafeedIndexed(torch.Tensor(X_train), torch.Tensor(y_train), 20, transform=None)
    for i in range(16):
        acquire_samples('5', trainset, 20)
